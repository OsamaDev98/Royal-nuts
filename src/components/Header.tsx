  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        isScrolled
          ? "py-2 backdrop-blur-xl shadow-2xl"
          : "py-4 bg-transparent"
      }`}
      style={isScrolled ? {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14">
          ...
          <Link href="/" className="flex items-center gap-3 group shrink-0">
            <div className="relative w-10 h-10 rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-[#C9A84C]/30 group-hover:shadow-xl transition-all duration-300"
              style={{ background: "linear-gradient(135deg,#C9A84C,#8B6820)" }}>
              <span className="text-xl">🥜</span>
            </div>
            <div className="flex flex-col leading-none">
              <span className="font-black text-[#F0EDE8] text-lg tracking-tight pb-1"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            style={{ background: "rgba(28,40,32,0.98)", borderBottom: "1px solid rgba(201,168,76,0.15)" }}
          >
